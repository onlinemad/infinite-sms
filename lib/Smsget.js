 ...
 * 
 * Supports both one-way and two-way SMS:
 * - One-way (default): Uses api_send.php
 * - Two-way: Uses api_send_twoway.php (set twoway: true in config)
 * 
...
class Smsget {
  constructor(cfg) {
    ...
    this.twoway = cfg.twoway || false // 是否使用雙向簡訊
    // 根據 twoway 設定 provider 名稱
    this.provider = this.twoway ? 'smsgetTwoway' : 'smsget'
  ...
  async send(payload) {
    if (!payload.to.startsWith('+886')) {
      throw new SmsError(this.provider, 'UNSUPPORTED_COUNTRY', 'Phone number must start with +886 (Taiwan only)', { error: { code: 'UNSUPPORTED_COUNTRY' } })
    }
    ...
    // 根據 twoway 設定選擇不同的 API endpoint
    const endpoint = this.twoway 
      ? `${API_ENDPOINT}/api_send_twoway.php`
      : `${API_ENDPOINT}/api_send.php`
    ...
    if (data.stats === true && data.error_code === '000') {
      ...
      throw new SmsError(this.provider, 'SEND_FAILED', message, smsRes?.rawJson || smsRes?.raw || smsRes)
    }
  }
